...
6. **`traits.bat`** собирает трейты (Rarity и др.) последних 200 минтов вместе с их digest
   в `runtime/traits.csv` и показывает, как редкость связана с хешем.
...
Во время работы `run` можно дописывать и удалять строки в `servers.txt`: через несколько секунд
после сохранения новые серверы подключаются, удалённые отключаются, остальные продолжают майнить.
