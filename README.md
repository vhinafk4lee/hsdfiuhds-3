...
```
scripts/protocols/*.json  one file per contract: chain, ABI, preimage layout
scripts/protocol.py     loads one of them, derives selectors, builds mine() calldata
...
python3 scripts/fleet.py start --wallet 0x... --blocks 16384     # start mining everywhere
...
105 tests, no GPU and no network needed. On Windows the ones that drive a fake
